...
import { useInvoiceColumns } from "./table/columns";
...
export function InvoiceList() {
  ...
  const { data: invoiceData, isLoading, isFetching } = useInvoices({ params });

  const data = useMemo(() => invoiceData?.data ?? [], [invoiceData]);
  const metadata = useMemo(() => invoiceData?.metadata, [invoiceData]);
  ...
  const table = useReactTable({
    ...
    getRowId: (row: InvoiceItem) => String(row.id),
    state: {
      pagination: {
