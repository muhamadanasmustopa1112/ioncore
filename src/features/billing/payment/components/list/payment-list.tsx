...
export function PaymentList() {
  ...
  const { data: paymentData, isLoading, isFetching } = usePayments({ params });

  const data = useMemo(() => paymentData?.data ?? [], [paymentData]);
  const metadata = useMemo(() => paymentData?.metadata, [paymentData]);
  ...
  const table = useReactTable({
    ...
    getRowId: (row: PaymentItem) => String(row.id),
    state: {
      pagination: {
