        ...
        <li key={h.work_order_id} className="flex items-start gap-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-100 dark:border-slate-800">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[10px] text-slate-400 mt-0.5">
              {humanize(h.type)} · {humanize(h.state)} · {fmtDate(h.completed_at)}
            </p>
          </div>
        </li>
      ))}
...
export function TimelineEntry({
  ...
  const fromLabel = t?.from_state ? STATE_LABEL[t.from_state] ?? humanize(t.from_state) : null;
  const toLabel = t?.to_state ? STATE_LABEL[t.to_state] ?? humanize(t.to_state) : null;
