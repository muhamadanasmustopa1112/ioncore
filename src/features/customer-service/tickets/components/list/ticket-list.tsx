...
import { useTickets } from "../../api/get-tickets";
import { useTicketColumns } from "./table/columns";
...
export function TicketList() {
  ...
  const { data: ticketData, isLoading, isFetching } = useTickets({ params });

  const data = useMemo(() => ticketData?.data ?? [], [ticketData]);
  const metadata = useMemo(() => ticketData?.metadata, [ticketData]);
            ...
            <CardHeading className="py-4">
              <div className="flex items-center gap-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    placeholder={t("cs.searchTickets", "Search tickets...")}
                    value={filter.search || ""}
                    onChange={(e) =>
              ...
              <CollapsibleContent>
                <div className="flex items-center gap-2 py-[5px] text-sm text-muted-foreground">
                  {t("cs.noFilters", "No advanced filters defined yet.")}
                </div>
              </CollapsibleContent>
            </CardHeading>
          </Collapsible>
