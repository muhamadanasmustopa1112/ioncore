...
export const createStatusIcon = (status: PopData['status'] | 'odp', isOdp = false) => {
  ...
  const svgIcon = isOdp ? `
    <svg width="24" height="32" viewBox="0 0 24 32" fill="none" xmlns="http://www.w3.org/2000/svg" style="filter: drop-shadow(0px 2px 4px rgba(0,0,0,0.3));">
  ...
  return L.divIcon({
    ...
    html: svgIcon,
    // Using L.point() is critical for Enterprise apps on Next.js 15+ 
    // to prevent "read-only property params" mutation errors.
