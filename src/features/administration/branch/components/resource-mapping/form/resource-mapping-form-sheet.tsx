...
import { ResourceMappingForm } from "./resource-mapping-form";

export function ResourceMappingFormSheet() {
  const { sheetOpen, closeSheet, form } = useResourceMappingStore();
  ...
  const handleSaveClick = () => {
    const submit = (window as unknown as Record<string, unknown>).__resourceMappingFormSubmit;
    ...
    <Sheet open={sheetOpen} onOpenChange={(open) => !open && closeSheet()}>
      <SheetContent className="inset-y-0 sm:inset-y-8 lg:end-10 start-auto h-full sm:max-h-[calc(100vh-64px)] gap-0 sm:rounded-lg border p-0 sm:max-w-none w-full md:w-[540px] lg:w-[650px] flex flex-col [&_[data-slot=sheet-close]]:end-5 [&_[data-slot=sheet-close]]:top-4.5 shadow-2xl">
        ...
        <SheetBody className="flex-1 p-0 overflow-hidden">
          <ResourceMappingForm onSubmit={closeSheet} />
        </SheetBody>
        ...
        <SheetFooter className="border-border flex-row gap-2.5 border-t p-5 pb-4 lg:gap-0 mt-auto">
          ...
          >
            {isNewMode ? "Add Mapping" : "Save Changes"}
          </Button>
        </SheetFooter>
      </SheetContent>
    </Sheet>
