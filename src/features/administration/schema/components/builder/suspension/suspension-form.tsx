...
export function SuspensionForm() {
  ...
  const [overrideSnapshot, setOverrideSnapshot] = useState<SuspensionFormValues>(DEFAULT_SUSPENSION);
  ...
  useEffect(() => {
    ...
    const cleaned = Object.fromEntries(Object.entries(partial).filter(([, v]) => v !== undefined)) as Partial<SuspensionFormValues>;
    const vals: SuspensionFormValues = { ...DEFAULT_SUSPENSION, ...cleaned, name: overrideCustomerSchema.schema_name ?? "_override" };
    reset(vals);
