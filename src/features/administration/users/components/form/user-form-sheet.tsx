...
export function UserFormSheet() {
  ...
  const getTitle = () => {
    if (isNewMode) return t("administration.users.addNewUserTitle");
    if (form === "edit") return t("administration.users.editUserTitle");
    return t("administration.users.userDetailsTitle");
  };

  const getDescription = () => {
    if (isNewMode) return "Create a new user account";
    if (form === "edit") return "Edit user account details";
