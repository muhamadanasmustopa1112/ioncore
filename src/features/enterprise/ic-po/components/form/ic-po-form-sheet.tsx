...
import { useCreateIcPo } from "../../api/post-ic-po";
import { useUpdateIcPo } from "../../api/put-ic-po";
import type { CreateIcPoPayload } from "../../types/ic-po";
import { IcPoForm, type IcPoFormRef } from "./ic-po-form";

export function IcPoFormSheet() {
  const { t } = useTranslation();
  const { form, sheetOpen, selectedItem, closeFormSheet } = useIcPoStore();
  const create = useCreateIcPo();
  const update = useUpdateIcPo();
  const isPending = create.isPending || update.isPending;
  const formRef = useRef<IcPoFormRef>(null);

  const handleSubmit = (payload: CreateIcPoPayload) => {
    if (form === "new") {
  ...
  const title =
    form === "new"
      ? t("enterprise.icPo.sheet.addTitle", "Create Intercompany PO")
      : form === "edit"
        ? t("enterprise.icPo.sheet.editTitle", "Edit Intercompany PO")
        : t("enterprise.icPo.sheet.detailTitle", "Intercompany PO Detail");
      ...
      >
        ...
        <SheetBody className="flex-1 p-0 overflow-hidden">
          <ScrollArea className="h-full">
            <IcPoForm ref={formRef} selected={selectedItem} mode={form ?? "new"} onSubmit={handleSubmit} />
          </ScrollArea>
        </SheetBody>
        ...
        <SheetFooter className="border-border flex-row gap-2.5 border-t p-5 pb-4 lg:gap-0 mt-auto shrink-0">
          ...
          <Button variant="primary" onClick={() => formRef.current?.submit()} disabled={form === "details" || isPending} className="font-semibold">
            {isPending ? t("common.saving", "Saving...") : form === "new" ? t("enterprise.icPo.sheet.create", "Create IC-PO") : t("common.saveChanges", "Save Changes")}
          </Button>
        </SheetFooter>
      </SheetContent>
    </Sheet>
