...
import { useResellers } from "../../api/get-resellers";
import { useResellerColumns } from "./table/columns";
...
export function ResellerList() {
  ...
  const { data: resellerData, isLoading, isFetching } = useResellers(params);

  const data = useMemo(() => resellerData?.resellers ?? [], [resellerData]);
  const total = resellerData?.metadata?.total ?? 0;
          ...
          <CardHeading className="py-4">
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <Input
                  placeholder={t("enterprise.resellers.searchPlaceholder", "Search resellers...")}
                  value={filter.search || ""}
