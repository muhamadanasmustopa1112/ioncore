...
import { usePreBoqs } from "../../api/get-pre-boqs";
import { usePreBoqColumns } from "./pre-boq-columns";

export function PreBoqList() {
  ...
  const columns = usePreBoqColumns();
  ...
  const { data, isLoading, isFetching } = usePreBoqs(params);

  const rows = useMemo(() => data?.pre_boqs ?? [], [data]);
        ...
        <CardHeader>
          <CardHeading className="py-4">
            <div className="relative">
              ...
              <Input
                placeholder={t("enterprise.cpq.searchPreBoq", "Search Pre-BoQ...")}
                value={filter.search || ""}
