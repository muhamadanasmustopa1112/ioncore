...
import { useBoqs } from "../../api/get-boqs";
import { useBoqColumns } from "./boq-columns";
...
export function BoqList() {
  ...
  const { data, isLoading, isFetching } = useBoqs(params);

  const rows = useMemo(() => data?.boqs ?? [], [data]);
        ...
        <CardHeader>
          <CardHeading className="py-4">
            <div className="relative">
              ...
              <Input
                placeholder={t("enterprise.cpq.searchBoq", "Search BoQ...")}
                value={filter.search || ""}
