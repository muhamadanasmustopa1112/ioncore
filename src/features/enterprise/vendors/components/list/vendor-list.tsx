...
import { useVendors } from "../../api/get-vendors";
import { useVendorColumns } from "./table/columns";
...
export function VendorList() {
  ...
  const { data: vendorData, isLoading, isFetching } = useVendors(params);

  const data = useMemo(() => vendorData?.vendors ?? [], [vendorData]);
  const total = vendorData?.metadata?.total ?? 0;
          ...
          <CardHeading className="py-4">
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <Input
                  placeholder={t("enterprise.vendors.searchPlaceholder", "Search vendors...")}
                  value={filter.search || ""}
