...
import { useSettlements } from "../../api/get-settlements";
import { useSettlementColumns } from "./table/columns";
...
export function SettlementList() {
  ...
  const params = useMemo(() => ({
    ...
    reseller_name: filter.search || undefined,
  }), [filter]);

  const { data: settlementData, isLoading, isFetching } = useSettlements(params);

  const data = useMemo(() => settlementData?.settlements ?? [], [settlementData]);
  const total = settlementData?.metadata?.total ?? 0;
          ...
          <CardHeading className="py-4">
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <Input
                  placeholder={t("enterprise.settlement.searchPlaceholder", "Search by reseller...")}
                  value={filter.search || ""}
