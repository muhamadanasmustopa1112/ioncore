...
export function ProjectList() {
  ...
  const { data: projectData, isLoading, isFetching } = useProjects(params);

  const data = useMemo(() => projectData?.projects ?? [], [projectData]);
  const total = projectData?.metadata?.total ?? 0;
          ...
          <CardHeading className="py-4">
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <Input
                  placeholder={t("enterprise.projects.searchPlaceholder", "Search projects...")}
                  value={filter.search || ""}
