...
import { useInventoryConfigs } from "../../api/get-inventory-configs";
import { useInventoryConfigColumns } from "./table/columns";
...
export function InventoryConfigList() {
  ...
  const columns = useInventoryConfigColumns();
  ...
  const { data: configData, isLoading, isFetching } = useInventoryConfigs({ params });

  const data = useMemo(() => configData?.data ?? [], [configData]);
  const metadata = useMemo(() => configData?.metadata, [configData]);
  ...
  const table = useReactTable({
    ...
    getRowId: (row: InventoryValuationConfig) => String(row.warehouseId),
    state: {
