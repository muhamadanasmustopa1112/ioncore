...
import { useReceiveColumns } from "./table/columns";
import { DataTableToolbar } from "./table/data-table-toolbar";
import { MobileReceiveHeader } from "./mobile-receive-header";
import { ReceiveMobileCard } from "./receive-mobile-card";
...
export function ReceiveList() {
  ...
  const { openReceiveFormSheet, setSelectedReceive } = useWarehouseStore();
  ...
  const columns = useReceiveColumns();
  ...
  const handleMobileDetail = useCallback(
    (item: StockLevel) => {
      ...
      openReceiveFormSheet("details");
    },
    [setSelectedReceive, openReceiveFormSheet]
  );

  const handleMobileReceive = useCallback(
    (item: StockLevel) => {
      setSelectedReceive(item);
      openReceiveFormSheet("new");
    },
    [setSelectedReceive, openReceiveFormSheet]
  );
    ...
    return (
      <div className="mt-2 space-y-3">
        ...
        <div className="relative">
          ...
          <Input
            placeholder={t("warehouse.searchReceive", "Search receive records...")}
            value={filter.search || ""}
        ...
        >
          ...
          {mobileData.length === 0 && !isLoading && (
            <div className="text-center py-8 text-muted-foreground text-sm">
              {t("warehouse.noReceiveItems", "No receive items found")}
            </div>
          )}
          ...
          {!hasMore && mobileData.length > 0 && (
            <div className="text-center py-3 text-[11px] text-muted-foreground">
              End of list • {filteredData.length} items
            </div>
          )}
        </div>
      </div>
