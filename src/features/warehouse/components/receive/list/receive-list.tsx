...
export function ReceiveList() {
  ...
  const filteredData = useMemo(() => {
    ...
    return DUMMY_STOCK_LEVELS.filter((item) => item.stockItemName.toLowerCase().includes(searchLower) || item.warehouseName.toLowerCase().includes(searchLower));
  }, [filter.search]);
            ...
            <CardHeading className="py-4">
              <div className="flex items-center gap-2">
                ...
                <div className="relative">
                  ...
                  <Input placeholder={t("warehouse.searchReceive", "Search receive records...")} value={filter.search || ""} onChange={(e) => setFilter({ ...filter, search: e.target.value })} className="w-64 ps-9" />
