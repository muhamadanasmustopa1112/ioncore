...
export function DispatchList() {
  ...
  const filteredData = useMemo(() => {
    if (!filter.search) return DUMMY_DISPATCHES;
    const searchLower = filter.search.toLowerCase();
    return DUMMY_DISPATCHES.filter((item) => item.woNumber.toLowerCase().includes(searchLower) || item.technicianName.toLowerCase().includes(searchLower));
  }, [filter.search]);
            ...
            <CardHeading className="py-4">
              <div className="flex items-center gap-2">
                ...
                <div className="relative">
                  ...
                  <Input placeholder={t("warehouse.searchDispatch", "Search dispatches...")} value={filter.search || ""} onChange={(e) => setFilter({ ...filter, search: e.target.value })} className="w-64 ps-9" />
