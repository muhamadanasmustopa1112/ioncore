...
import { TransferFormSheet } from "./form/transfer-form-sheet";
      ...
      <Toolbar className={isMobile ? "mt-2 items-center" : "mt-5 items-center"}>
        <ToolbarHeading>
          <ToolbarTitle className={isMobile ? "text-lg font-extrabold tracking-tight" : "text-2xl font-extrabold tracking-tight"}>
            {t("warehouse.transfersTitle", "Transfers")}
          </ToolbarTitle>
        </ToolbarHeading>
        <ToolbarActions>
          {!isMobile && (
          ...
          <Button variant="primary" className={isMobile ? "h-10 px-4 font-semibold shadow-md" : "h-11 px-6 font-semibold shadow-md"} onClick={() => openTransferFormSheet("new")}>
            <RiAddLine className={isMobile ? "size-4" : "size-5"} />
            {t("warehouse.newTransfer", "New Transfer")}
          </Button>
        </ToolbarActions>
      </Toolbar>
