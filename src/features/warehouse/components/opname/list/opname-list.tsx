...
export function OpnameList() {
  ...
  const filteredData = useMemo(() => {
    if (!filter.search) return DUMMY_OPNAMES;
    const searchLower = filter.search.toLowerCase();
    return DUMMY_OPNAMES.filter((item) => item.warehouseName.toLowerCase().includes(searchLower) || item.initiatedByName.toLowerCase().includes(searchLower));
  }, [filter.search]);
            ...
            <CardHeading className="py-4">
              <div className="flex items-center gap-2">
                ...
                <div className="relative">
                  ...
                  <Input placeholder={t("warehouse.searchOpname", "Search opname...")} value={filter.search || ""} onChange={(e) => setFilter({ ...filter, search: e.target.value })} className="w-64 ps-9" />
